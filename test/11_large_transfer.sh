...
log "=== TCP 10MB echo ==="
START=$(date +%s)
nc -w 60 "$TUN_IP" "$PORT" < "$TMPDIR/10m" > "$TMPDIR/10m_out" 2>/dev/null
...
log "=== TCP 50MB echo ==="
START=$(date +%s)
nc -w 180 "$TUN_IP" "$PORT" < "$TMPDIR/50m" > "$TMPDIR/50m_out" 2>/dev/null
ELAPSED=$(( $(date +%s) - START ))
...
START=$(date +%s)
nc -w 120 "$TUN_IP" "$PORT" < "$TMPDIR/10m" > "$TMPDIR/10m_loss3" 2>/dev/null
ELAPSED=$(( $(date +%s) - START ))
