...
for i in $(seq 1 $N); do
    (nc -w 10 "$TUN_IP" "$PORT" < "$TMPDIR/64k" > "$TMPDIR/stress1/$i" 2>/dev/null) &
done
wait
OK=0
for i in $(seq 1 $N); do
...
for i in $(seq 1 $N); do
    (nc -w 10 "$TUN_IP" "$PORT" < "$TMPDIR/16k" > "$TMPDIR/stress2/$i" 2>/dev/null) &
done
wait
OK=0
for i in $(seq 1 $N); do
...
for i in $(seq 1 $N); do
    (nc -w 30 "$TUN_IP" "$PORT" < "$TMPDIR/64k" > "$TMPDIR/stress3/$i" 2>/dev/null) &
done
wait
